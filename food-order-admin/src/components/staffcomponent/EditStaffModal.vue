...
export default {
  ...
  setup(props, { emit }) {
    ...
    const initForm = () => {
      if (props.staff) {
        formData.fullName = props.staff.fullName || ''
        formData.email = props.staff.email || ''
        formData.phoneNumber = props.staff.phoneNumber || ''
        formData.password = ''
      }
    }
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        if (changePassword.value && formData.password) {
          payload.password = formData.password
        }

        await userStore.editStaff(props.staff.id, payload)
        emit('success')
      } catch (error) {
        ...
        if (error.errors) {
          ...
          submitError.value = error.message || 'Có lỗi xảy ra khi cập nhật nhân viên'
        }
      } finally {
        loading.value = false
      }
